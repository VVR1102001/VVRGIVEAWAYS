...
$cname = "Default Lifafa Link page";
...
*/
     ...
     // echo $url;
      ...
      if($urlid==null||$urlid==" "){
          ...
          echo "ACCESS DENIED";
          //echo "<title>vvrgiveaways - default lifafa page</title>";
...
if(isset($_SESSION["LTID"])){
    $tid = $_SESSION["LTID"];
}
$usql = "select UID from uv where IP='$ip' and TID = '$tid' ";
$ures = mysqli_query($fconn,$usql);
if(mysqli_num_rows($ures)>0){
    $ufetch = mysqli_fetch_assoc($ures);
    $uid = $ufetch["UID"];
    $fsql = "select * from uv where LIFID='$urlid' and IP = '$ip' and UID ='$uid' and TID = '$tid'";
